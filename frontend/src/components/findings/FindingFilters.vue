  ...
  <div class="filters-container">
    ...
    <div class="severity-quick-filters">
      ...
      >
        ...
        <span class="severity-count">{{ getSeverityCount(sev.value) }}</span>
      </button>
    </div>
    ...
    <div class="filters-row">
      ...
      <div class="filter-group">
        <label>MASVS Category</label>
        <Dropdown
          v-model="selectedMasvs"
          :options="masvsOptions"
          optionLabel="label"
          optionValue="value"
          placeholder="All Categories"
          showClear
          @change="handleMasvsChange"
        />
      </div>
...
const selectedSeverities = ref<string[]>(
  findingsStore.filters.severity ? [...findingsStore.filters.severity] : []
)
const selectedStatuses = ref<string[]>(
  findingsStore.filters.status ? [...findingsStore.filters.status] : []
)
const selectedTool = ref<string | null>(
  findingsStore.filters.tool?.[0] || null
)
const selectedMasvs = ref<string | null>(
  findingsStore.filters.owasp_masvs_category?.[0] || null
)
...
const toolOptions = computed(() => {
  const tools = findingsStore.filterOptions?.tools || []
  return tools.map((t: string) => ({ label: t, value: t }))
})

const masvsOptions = computed(() => {
  const categories = findingsStore.filterOptions?.masvs_categories || []
  return categories.map((c: string) => ({ label: c, value: c }))
})

const hasFilters = computed(() => findingsStore.hasFilters)

const getSeverityCount = (severity: string): number => {
  return findingsStore.summary?.by_severity?.[severity] || 0
}
...
const handleSeverityChange = () => {
  findingsStore.setFilters({
    severity: selectedSeverities.value.length > 0 ? selectedSeverities.value : null,
  })
  emit('filter-change')
}
...
const handleToolChange = () => {
  findingsStore.setFilters({
    tool: selectedTool.value ? [selectedTool.value] : null,
  })
  emit('filter-change')
}

const handleMasvsChange = () => {
  findingsStore.setFilters({
    owasp_masvs_category: selectedMasvs.value ? [selectedMasvs.value] : null,
  })
  emit('filter-change')
}
...
.severity-quick-btn.critical .severity-label {
  color: var(--severity-critical);
}
.severity-quick-btn.high .severity-label {
  color: var(--severity-high);
}
.severity-quick-btn.medium .severity-label {
  color: var(--severity-medium);
}
.severity-quick-btn.low .severity-label {
